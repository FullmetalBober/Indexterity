...
| `config.workerPollIntervalMs` | How long an idle job runner waits, in ms, before asking Postgres whether anything became due. Empty is 10 000. Polling is the fallback path — an enqueued job arrives by `NOTIFY` and starts at once regardless — so raising this delays only clock-driven work (a cron tick, an expired retry backoff). Worth setting on metered Postgres: 10s is ~260k round trips a month from one idle worker, 60s is ~43k. Applies to the worker **and** to an `api.runWorker` install |
