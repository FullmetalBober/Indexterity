...
| `config.clusterIndexBuildTimeoutMs` | How long one **index build** may run before the engine cancels it — 7200000 (2h). Its own budget because a catalog read that has not answered in 15 minutes is hung while a `CREATE INDEX` that has run 15 minutes may be half done; they shared one number until #410, sized for the read, so a build that genuinely took an hour could not finish. **Keep it under four hours** or the queue reclaims the job's lock mid-build. Postgres and SQL Server only — MongoDB's driver socket has no timeout and is left that way |
