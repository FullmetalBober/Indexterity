...
vi.mock("~/lib/api", () => ({ api: () => ({ listClusters, getOrg, listOrgs }) }));
...
// DOM. useShell is a hook over three queries, so it needs a component to live in.
function Probe() {
...
describe("useShell", () => {
  it("is signed in once all three reads land", async () => {
    renderInApp(<Probe />);
  ...
  it.each(["clusters", "org", "orgs"] as const)(
  ...
  // auth gate is deliberately not: it needs all three before it will call someone
  // signed in, or the layout would draw a cluster bar above a missing org.
  it("waits for all three rather than deciding on the first", async () => {
