...
// Constructed over a fake pool, not booted: the service holds the Database and
...
describe("the dial budget", () => {
  it("allows the attempt that spends the last unit", async () => {
    await expect(budgetAt(10, 41).consume("user-1")).resolves.toBeUndefined();
  });
